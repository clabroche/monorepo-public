      ...
      <div class="content-box">
        <div class="inputs">
          ...
          <div class="input-container">
            ...
            <input type="password" :value="user.password" @input="user.password = $event.target?.value"
              placeholder="Mot de passe" @keypress.enter="register ? registerFun() : login()">
          </div>
        </div>
        <div class="action">
          <button v-if="!register" @click="login" :disabled="!user.email || !user.password">Se connecter <i
              class="fas fa-sign-in-alt" aria-hidden="true"></i></button>
          <button v-else @click="registerFun" :disabled="!user.email || !user.password">S'enregistrer <i
              class="fas fa-sign-in-alt" aria-hidden="true"></i></button>
        </div>
        <div class="switch">
          <div v-if="!register">Vous n'avez pas de compte ? <router-link to="register">Enregistrez-vous</router-link>
          </div>
          <div v-else>Vous avez déjà un compte ?<router-link to="login">Connectez-vous</router-link>
          </div>
        </div>
      </div>
    </div>
