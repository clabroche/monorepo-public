...
describe('Gulp > additionalDeps', () => {
  it('getAdditionalDeps', async () => {
    ...
    expect(additionalDepsRes[0].name).toEqual('@clabroche-org/common-express-http-error')
    expect(additionalDepsRes[0].name).toEqual('@clabroche-org/common-express-http-error')
  })
  it('getPkgJSON', async () => {
    const additionalDepsRes = await additionalDeps.getPkgJSON(pathfs.resolve(__dirname, '..', '..', 'express-logger'))
    expect(additionalDepsRes.name()).toEqual('@clabroche-org/common-express-logger')
